...
func init() {
	plugin.Regist("weak-passwords", &wordPressWeak{})
}
...
func (d *wordPressWeak) Check(URL string, meta plugin.TaskMeta) bool {
	...
	for _, user := range userList {
		for _, pass := range meta.PassList {
			...
			request, err := http.NewRequest("POST", URL+"/wp-login.php?loggedout=true", strings.NewReader(fmt.Sprintf(postData, user, pass)))
